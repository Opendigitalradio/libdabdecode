...
namespace dabdecode
  ...
  void fig_1::dispatch(ensemble & target) const
    {
    target.handle(*this);
    }
  ...
  void fig_1::parse_extension()
    ...
    switch(m_dataField[0] & 3)
      {
      case 0:
        m_extension = new extension_0{uint16_t((((uint16_t) m_dataField[1]) << 8) | m_dataField[2])};
        break;
      case 1:
        m_extension = new extension_1{uint16_t((((uint16_t) m_dataField[1]) << 8) | m_dataField[2])};
        break;
      case 5:
        m_extension = new extension_5{(uint32_t((((uint32_t) m_dataField[1]) << 24) |
